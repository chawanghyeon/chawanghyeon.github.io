...
export function normalizeSteps(rawSteps: unknown[]): Step[] {
    ...
    return rawSteps.map((s: unknown) => {
        ...
        return {
            ...
            options: ((step.options as unknown[]) || []).map(
                (o: unknown, i: number) => {
                    const option = o as Record<string, unknown>;
                    return {
                        id:
                            (option.id as string) ??
                            generateId(ID_PREFIXES.OPTION),
...
export class WorkflowStorage {
    ...
    static exportData(
        ...
        filename: string = "workflow-data.json"
    ) {
...
export function calculateStepCombinationStats(
    ...
    return steps.map((_, stepIdx) => {
        ...
        const enabledPercentage =
            totalForStep > 0
                ? Math.round((enabledForStep / totalForStep) * 100)
                : 0;
...
export function calculateStepOptionStats(
    ...
    return steps.map((step, stepIdx) => {
        ...
        return stepOptions.map((option) => {
            ...
            const enabledPercentage =
                totalForOption > 0
                    ? Math.round((enabledForOption / totalForOption) * 100)
                    : 0;
