...
export class StorageManager {
    ...
    static async initializeFileSystem(
        suggestedName: string = "workflow-data.json"
    ): Promise<boolean> {
    ...
    static async setItem(
        ...
        if (useFileSystem && this.isFileSystemSupported()) {
            if (!this.fileHandle) {
                const initialized = await this.initializeFileSystem(
                    `${key}.json`
                );
                if (!initialized) {
                    console.warn(
                        "Failed to initialize file system, falling back to localStorage"
                    );
                    return this.setItemLocalStorage(key, data, {
                        backup,
                        maxRetries,
                    });
                }
     ...
     */
    private static setItemLocalStorage(
        key: string,
        data: unknown,
        options: StorageOptions = {}
    ): boolean {
            ...
            for (let attempt = 0; attempt < maxRetries; attempt++) {
                try {
                    ...
                    if (
                        ...
                        throw new Error(
                            `Storage quota exceeded. Used: ${this.getStorageInfo().usedPercentage.toFixed(
                                1
                            )}%`
                        );
                    }
    ...
    static performCleanup(protectedKey?: string): number {
        ...
        try {
            ...
            for (const key in localStorage) {
                if (localStorage.hasOwnProperty(key)) {
                    ...
                    if (
                        ...
                        key !== protectedKey + this.BACKUP_SUFFIX
                    ) {
            ...
            if (freedSpace < 1024 * 1024) {
                // Less than 1MB freed
                const workflowKeys = Object.keys(localStorage).filter(
                    (key) =>
     ...
     */
    static async enableFileSystemMode(
        suggestedName?: string
    ): Promise<boolean> {
    ...
    static async exportToFile(
        data: unknown,
        suggestedName: string = "workflow-export.json"
    ): Promise<boolean> {
        if (!this.isFileSystemSupported()) {
            console.warn("File System Access API not supported");
            return false;
