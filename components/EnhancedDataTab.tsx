...
const EnhancedDataTab: React.FC<EnhancedDataTabProps> = ({
    ...
    const currentSheetStats = React.useMemo(() => {
        ...
        const optionCount = currentSheet.steps.reduce(
            (sum, step) => sum + step.options.length,
            0
        );
        const constraintCount = Object.keys(
            currentSheet.constraints || {}
        ).length;
        ...
        return {
            ...
            optionCount,
            constraintCount,
            totalCombinations: currentSheet.steps.reduce(
                (product, step) => product * Math.max(1, step.options.length),
                1
            ),
        };
    }, [currentSheet]);
    ...
    const overallStats = React.useMemo(() => {
        ...
        const totalSteps = allSheets.reduce(
            (sum, sheet) => sum + sheet.steps.length,
            0
        );
        const totalOptions = allSheets.reduce(
            (sum, sheet) =>
                sum +
                sheet.steps.reduce(
                    (stepSum, step) => stepSum + step.options.length,
                    0
                ),
            0
        );
        const totalConstraints = allSheets.reduce(
            (sum, sheet) => sum + Object.keys(sheet.constraints || {}).length,
            0
        );
    ...
    const handleClearData = () => {
        if (
            confirm(
                "정말로 모든 워크플로우 데이터를 삭제하시겠습니까? 이 작업은 되돌릴 수 없습니다."
            )
        ) {
                    ...
                    <div className="current-sheet-stats">
                        <div className={styles.statsGrid}>
                            ...
                            <div className={styles.statItem}>
                                ...
                                <span className="stat-value">
                                    {currentSheetStats.stepCount}
                                </span>
                            </div>
                            <div className={styles.statItem}>
                                <span className="stat-label">옵션 수:</span>
                                <span className="stat-value">
                                    {currentSheetStats.optionCount}
                                </span>
                            </div>
                            <div className={styles.statItem}>
                                <span className="stat-label">정책 수:</span>
                                <span className="stat-value">
                                    {currentSheetStats.constraintCount}
                                </span>
                            </div>
                            <div className={styles.statItem}>
                                ...
                                <span className="stat-value">
                                    {currentSheetStats.totalCombinations.toLocaleString()}
                                </span>
                            </div>
                        </div>
                    </div>
                ...
                <div className="overall-stats">
                    <div className={styles.statsGrid}>
                        ...
                        <div className={styles.statItem}>
                            ...
                            <span className="stat-value">
                                {overallStats.totalOptions}
                            </span>
                        </div>
                        <div className={styles.statItem}>
                            <span className="stat-label">총 정책 수:</span>
                            <span className="stat-value">
                                {overallStats.totalConstraints}
                            </span>
                        </div>
                    </div>
                </div>
