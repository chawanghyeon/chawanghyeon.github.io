...
import {
    ...
    adjustConstraintIndicesForStepMove,
...
export const useSheetManager = () => {
    ...
    const [constraintNotification, setConstraintNotification] = useState<
        string | null
    >(null);
    ...
    const generateCurrentPathActivations = useCallback(() => {
        const activeSheet = workflowData.sheets.find(
            (sheet) => sheet.id === workflowData.activeSheetId
        );
    ...
    const updateCurrentSheet = useCallback(
        (
            updater: (sheet: WorkflowSheet) => WorkflowSheet,
            skipPathRecalc = false
        ) => {
            setWorkflowData((prev) => {
                const activeSheetIndex = prev.sheets.findIndex(
                    (sheet) => sheet.id === prev.activeSheetId
                );
    ...
    const getConstraintDescription = useCallback(
        (constraint: WorkflowConstraint): string => {
            const sourceStep = steps[constraint.sourceStepIndex];
            const sourceOption = sourceStep?.options.find(
                (opt) => opt.id === constraint.sourceOptionId
            );
            const stepName =
                sourceStep?.displayName ||
                sourceStep?.name ||
                `${constraint.sourceStepIndex + 1}단계`;
            const optionName =
                sourceOption?.displayName || sourceOption?.name || "옵션";
            return `${stepName} - ${optionName}: 정책`;
        },
        [steps]
    );
    ...
    const findAffectedConstraints = useCallback(
        (
            operation: "addStep" | "deleteStep" | "addOption" | "deleteOption",
            params: {
                insertIndex?: number;
                stepIndex?: number;
                optionId?: string;
    ...
    const checkAffectedConstraintsAndConfirm = useCallback(
        (
            operation: "addStep" | "deleteStep" | "addOption" | "deleteOption",
            params: {
                insertIndex?: number;
            ...
            const affectedConstraints = findAffectedConstraints(
                operation,
                params
            );
            ...
            const message =
                `${operationText}하면 ${affectedConstraints.length}개의 정책이 영향을 받아 사용 불가 상태가 됩니다:\n\n` +
                affectedConstraints
                    .map((c) => `• ${getConstraintDescription(c)}`)
                    .join("\n") +
                `\n\n계속하시겠습니까? (정책은 삭제되지 않고 정책 관리자에서 '사용 안함' 상태로 표시됩니다)`;
    ...
    const copySheet = useCallback(
        (sheetId: string, newName?: string) => {
            const sourceSheet = workflowData.sheets.find(
    ...
    const addStepAtIndex = useCallback(
        (insertIdx: number) => {
            // Check for affected constraints first
            if (
                !checkAffectedConstraintsAndConfirm("addStep", {
            ...
            updateCurrentSheet((sheet) => {
                ...
                const affectedConstraints = findAffectedConstraints("addStep", {
                    insertIndex: insertIdx,
                });
                const affectedConstraintIds = new Set(
                    affectedConstraints.map((c) => c.id)
                );
                ...
                if (affectedConstraintIds.size > 0) {
                    setConstraintNotification(
                        `단계 추가로 인해 ${affectedConstraintIds.size}개의 정책이 사용 불가 상태가 되었습니다.`
                    );
        ...
        [
            updateCurrentSheet,
            checkAffectedConstraintsAndConfirm,
    ...
    const deleteStep = useCallback(
        (stepId: string) => {
            updateCurrentSheet((sheet) => {
                const stepIndex = sheet.steps.findIndex(
                    (step) => step.id === stepId
                );
                ...
                // Check for affected constraints first
                if (
                    !checkAffectedConstraintsAndConfirm("deleteStep", {
                        stepIndex,
                    })
                ) {
                ...
                // Remove the step
                const updatedSteps = sheet.steps.filter(
                    (step) => step.id !== stepId
                );
                ...
                if (affectedConstraintIds.size > 0) {
                    setConstraintNotification(
                        `단계 삭제로 인해 ${affectedConstraintIds.size}개의 정책이 사용 불가 상태가 되었습니다.`
                    );
                    setTimeout(() => setConstraintNotification(null), 5000);
                }
        ...
        (fromIndex: number, toIndex: number) => {
            updateCurrentSheet((sheet) => {
                if (
                    ...
                    fromIndex < 0 ||
                    toIndex < 0 ||
                    fromIndex >= sheet.steps.length ||
                    toIndex >= sheet.steps.length
                ) {
                    return sheet;
                }
                ...
                if (constraintCount > 0) {
                    const confirmed = window.confirm(
                        `단계를 이동하면 ${constraintCount}개의 정책의 인덱스가 자동으로 업데이트됩니다.\n` +
                            `이동하시겠습니까?`
                    );
                    if (!confirmed) {
                        return sheet; // 사용자가 취소
                    }
                }
                ...
                // 정책의 인덱스도 업데이트
                const adjustedConstraints = adjustConstraintIndicesForStepMove(
                    sheet.constraints || {},
                    fromIndex,
                    toIndex
                );
                ...
                if (constraintCount > 0) {
                    setConstraintNotification(
                        `단계 이동 완료. ${constraintCount}개의 정책이 업데이트되었습니다.`
                    );
                    setTimeout(() => setConstraintNotification(null), 3000);
                }
    ...
    const deleteOption = useCallback(
        (stepId: string, optionId: string) => {
            updateCurrentSheet((sheet) => {
                ...
                // Check for affected constraints first
                if (
                    !checkAffectedConstraintsAndConfirm("deleteOption", {
                        stepIndex,
                        optionId,
                    })
                ) {
                ...
                if (affectedConstraintIds.size > 0) {
                    setConstraintNotification(
                        `옵션 삭제로 인해 ${affectedConstraintIds.size}개의 제약 조건이 사용 불가 상태가 되었습니다.`
                    );
                    setTimeout(() => setConstraintNotification(null), 5000);
                }
    ...
    const toggleStepActive = useCallback(
    ...
    const toggleOptionActive = useCallback(
        () => {
            ...
            console.warn(
                "toggleOptionActive called but pathActivations are now generated dynamically"
            );
        },
        []
    );

    const toggleOptionByStepAndOption = useCallback(
        (stepId: string, optionId: string, isActive: boolean) => {
            updateCurrentSheet((sheet) => ({
                ...sheet,
                steps: sheet.steps.map((step) =>
                    step.id === stepId
    ...
    const updateConstraint = useCallback(
        (constraintId: string, updates: Partial<WorkflowConstraint>) => {
            updateCurrentSheet((sheet) => ({
                ...sheet,
                constraints: {
                    ...sheet.constraints,
    ...
    const adjustConstraintIndices = useCallback(
        (insertIndex: number) => {
            updateCurrentSheet((sheet) => {
                const adjustedConstraints =
                    adjustConstraintIndicesForStepInsertion(
